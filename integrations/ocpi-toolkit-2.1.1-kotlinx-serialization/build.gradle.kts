...
dependencies {
    ...
    implementation(project(":ocpi-toolkit-2.1.1"))
...
publishing {
    publications {
        named<MavenPublication>("maven") {
            artifactId = "ocpi-2-1-1-kotlinx-serialization"
            from(components["java"])

            pom {
                name.set("OCPI 2.1.1 kotlinx serialization")
                artifactId = "ocpi-2-1-1-kotlinx-serialization"
                description.set("Kotlinx Serialization serializers for OCPI 2.1.1 data classes")
            }
        }
    }
}
