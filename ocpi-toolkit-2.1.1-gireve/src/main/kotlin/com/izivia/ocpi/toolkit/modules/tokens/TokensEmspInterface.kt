...
interface TokensEmspInterface {
     ...
     *
     * Gireve notes:
     * If « LocationReferences » object contains 0 EVSE, IOP responds with a 2002 OCPI error (« Missing EVSEd »).
     * If « LocationReferences » object contains more than 1 EVSE, IOP responds with a 2001 OCPI error (« Invalid or
     * missing parameters: IOP does not support authorization request on multiple EVSE »).
     *
     * @param tokenUid (max-length 36) 	Token.uid of the Token for which this authorization is.
     * @param tokenType Token.type of the Token for which this authorization is. Default if omitted: RFID
     * @param locationReferences Location and EVSE/Connectors for which the Token is requested to be authorized. Must
     * contain one and only one EVSE
